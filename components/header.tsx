  ...
  return (
    <>
      <motion.div
        ...
        shadow-lg shadow-black/[0.03] backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.25rem] sm:w-[27rem] sm:rounded-full"
        // dark:bg-gray-950 dark:border-black/40 dark:bg-opacity-75
        initial={{ y: -100, x: '-50%', opacity: 0 }}
        animate={{ y: 0, x: '-50%', opacity: 1 }}
      />
      <nav className="flex fixed top-[0.8rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[2.1rem] sm:h-[initial] sm:py-0">
          ...
          {links.map((link) => (
            ...
            >
              <Link
                href={link.hash}
                className={clsx(
                  'relative px-2 py-1 z-999 font-bold transition hover:text-gray-900',
                  {
